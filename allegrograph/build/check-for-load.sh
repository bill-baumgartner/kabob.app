...
# detected then an AllegroGraph bulk load is initiated.  The files in
...
inotifywait -m $1 -e create,moved_to,attrib |
	...
	then
	    ...
	    if [[ ! -z "$SUPERSEDE_FLAG" ]]
	    then
		PARAMS="$PARAMS --supersede"
	    fi

	    load_command="/app/agraph/bin/agload --verbose --bulk --graph :source --port $AG_PORT --duplicates delete $PARAMS $REPO_NAME @$path$file"

	    echo "EXECUTING LOAD COMMAND: $load_command" 
	    
	    su agraph -c "$load_command" | tee $path$file.log
