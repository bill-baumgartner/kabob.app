...
docker run --rm --net agraph-net-${KB_KEY} --volumes-from kabob_data-${KB_KEY} --volumes-from ag-load-requests-${KB_KEY} billbaumgartner/kabob-base:0.3 /kabob.git/scripts/docker/allegrograph-specific/delete-duplicate-triples-ag.sh ${KB_NAME}
